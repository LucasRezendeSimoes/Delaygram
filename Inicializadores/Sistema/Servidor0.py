...
sync_socket.setsockopt(zmq.IDENTITY, b'server0')  # Altere conforme o ID do servidor
sync_socket.connect("tcp://localhost:5560")

gerente_atual = b'server1'
#-----------------------------------------------------------------------------------
print("\033[34mServer - 0")
...
def heartbeat():
    ...
    socketh.setsockopt(zmq.IDENTITY, b'server0')  # Define identidade clara
    socketh.connect("tcp://localhost:5559")

    while True:
        socketh.send_multipart([b'server0', b'HEARTBEAT'])
        print("[HEARTBEAT ENVIADO] Para broker: server0 - HEARTBEAT")
        sleep(5)
